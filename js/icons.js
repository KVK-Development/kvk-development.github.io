// Auto-generated inline icon set, sourced from lucide-static (ISC license).
// Each entry is the inner <path>/<rect>/<circle> markup for one icon,
// keyed by lucide's kebab-case icon name.
var ICONS = {
...
function renderIcon(name, opts) {
  ...
  return (
    ...
    strokeWidth + '" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">' +
    markup +
    '</svg>'
  );
}
...
function applyIcons(root) {
  ...
  var placeholders = scope.querySelectorAll('svg[data-icon]');
  placeholders.forEach(function (node) {
    var name = node.getAttribute('data-icon');
    var size = node.getAttribute('data-size') || 24;
    var strokeWidth = node.getAttribute('data-stroke-width') || 2;
    var markup = ICONS[name];
    if (!markup) return;
